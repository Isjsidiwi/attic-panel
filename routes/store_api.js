...
router.get('/order/:id/status', async (req, res) => {
  try {
    const { rows } = await db.execute(
      `SELECT o.*, k.key_value FROM store_orders o LEFT JOIN store_keys k ON k.id = o.key_id WHERE o.id = ?`,
      [req.params.id]
    );
    ...
    if (paid) {
      ...
      const { rows: keyRows } = await db.execute(
        `SELECT id, key_value FROM store_keys WHERE variant_id = ? AND is_used = 0 LIMIT 1`,
        [order.variant_id]
      );
      ...
      if (!keyRows.length) {
        ...
        await db.execute(
          `UPDATE store_orders SET status = 'paid', paid_at = datetime('now','localtime') WHERE id = ?`,
          [order.id]
        );
      ...
      await db.execute(
        `UPDATE store_keys SET is_used = 1, order_id = ?, used_at = datetime('now','localtime') WHERE id = ?`,
        [order.id, key.id]
      );
      await db.execute(
        `UPDATE store_orders SET status = 'paid', paid_at = datetime('now','localtime'), key_id = ? WHERE id = ?`,
        [key.id, order.id]
      );
