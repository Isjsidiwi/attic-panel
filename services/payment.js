...
const OK_CONFIG = {
  ...
  app_reg_id:
    process.env.OK_APP_REG_ID ||
    'eaAxVAUUR3mL01mAZA6OY-:APA91bHLfygS2CD8pLwfhR3NGVCS7-HGQJEpRgkohRTyJdzRkszvXSMyigO3Dq4kYRnMdf6p5jVIUyBkcf7XqN2LUMQs-JhGGdvH-ZPvSqEl58ikoD0LjAQ',
...
async function createQRIS(amount) {
  ...
  const response = await makeProxyRequest('https://app.orderkuota.com/api/v2/get', 'POST', headers, data);
...
async function checkPayment(uniqueAmount, createdAt) {
  try {
    ...
    for (let page = 1; page <= pagesToCheck; page++) {
      ...
      console.log(
        `[PaymentCheck] Checking ${uniqueAmount} after ${createdDate.toISOString()}. Page ${page}, mutations: ${results.length}`
      );
      ...
      for (const trx of results) {
        ...
        if (trxAmount === Number(uniqueAmount) && (!status || status === 'IN' || status === 'KREDIT')) {
          ...
          console.log(
            `[PaymentCheck] Found match amount ${trxAmount}. TrxDate: ${trxDate.toISOString()}, Recent: ${isRecent}`
          );
