    ...
    <div class="col-lg-6">
        ...
        <div class="card" style="background: rgba(20, 20, 20, 0.8); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 20px; backdrop-filter: blur(10px); position: relative; overflow: hidden; box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.37), 0 0 80px rgba(99, 102, 241, 0.1);">
            ...
            <div class="card-header mb-3 p3 text-white" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border-bottom: 1px solid rgba(99, 102, 241, 0.2); border-radius: 20px 20px 0 0;">
                <div class="row">
                    ...
                    <div class="col text-end">
                       ...
                       <a class="btn btn-outline-light btn-sm" href="<?= site_url('keys/generate') ?>"><i class="fa-solid fa-file-plus fa-flip" style="--fa-animation-duration: 8s; color: #00ffbb;"> </i></a>
            ...
            <div class="card-body" style="background: transparent; color: #e6eef8;">
                  ...
                  <label for="custom" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">SELECT KEY TYPE</label>
                        <?php $sel_type = ['normal' => 'KEY WITH NAME', 'custom' => 'ONLY NAME KEY',]; ?>
                <?= form_dropdown(['class' => 'form-select', 'name' => 'custom', 'id' => 'custom'], $sel_type ) ?>
                </div>
                </div>
                
                <div id="form-2" style="display: block;">
                <?= form_open() ?>
               ...
               <label for="max_devices" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">Max Devices</label>
                 <input type="number" name="max_devices" id="max_devices" class="form-control" placeholder="1" value="<?= old('max_devices') ?: 1 ?>" min="1" max="1000" required>
                        <?php if ($validation->hasError('game')) : ?>
              ...
              <div class="form-group col-lg-6 mb-3">
              <label for="name" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">ADD NAME</label>
                  <input type="text" class="form-control" name="name" id="name" aria-describedby="help-name" placeholder="ENTERNAME" minlength="4" maxlength="20" required>
                    <?php if ($validation->hasError('name')) : ?>
                        <small id="help-name" class="form-text text-danger"><?= $validation->getError('name') ?></small>
                    <?php endif; ?>
                </div>
      
                         <?php if (($user->level == 1) || ($user->level ==2)) { ?>
            <div class="form-group mb-3">
                       <label for="bulk" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">ENTER KEYS COUNT</label>
             ...
             <?php } else { ?>
                   <div class="form-group mb-3">
               <label for="bulk" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">ENTER BULK KEY</label>
        ...
        <?= form_open() ?>
         <input type="hidden" name="normal" value="1">
                <div class="row">
                    <div class="form-group col-lg-6 mb-3">
                        <label for="game" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">Games</label>
                        <?= form_dropdown(['class' => 'form-select', 'name' => 'game', 'id' => 'game_2'], $game, old('game') ?: '') ?>
                        <?php if ($validation->hasError('game')) : ?>
                            <small id="help-game" class="text-danger"><?= $validation->getError('game') ?></small>
                        <?php endif; ?>
                    </div>
                   ...
                   <div class="form-group col-lg-6 mb-3">
                    <label for="duration" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">Duration Of Key</label>
                    <?= form_dropdown(['class' => 'form-select', 'name' => 'duration', 'id' => 'duration_2'], $duration, old('duration') ?: '') ?>
                    <?php if ($validation->hasError('duration')) : ?>
                        <small id="help-duration" class="text-danger"><?= $validation->getError('duration') ?></small>
                    <?php endif; ?>
                        </div>    
                    ...
                    <div class="form-group col-lg-6 mb-3">
                <label for="max_devices" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">Max Devices</label>
                  <input type="number" name="max_devices" id="max_devices_2" class="form-control" placeholder="1" value="<?= old('max_devices') ?: 1 ?>" min="1" max="100" required>
                        <?php if ($validation->hasError('game')) : ?>
                            <small id="help-max_devices" class="text-danger"><?= $validation->getError('max_devices') ?></small>
                                <?php endif; ?>
                                    </div>
                         ...
                         <div class="form-group col-lg-6 mb-3">
                     <label for="name" class="form-label" style="color: rgba(255, 255, 255, 0.8); font-weight: 500;">ADD NAME</label>
                    <input type="text" class="form-control" name="name" id="name" aria-describedby="help-name" placeholder="ENTERNAME" minlength="4" maxlength="30" required>
                    <?php if ($validation->hasError('name')) : ?>
                        <small id="help-name" class="form-text text-danger"><?= $validation->getError('name') ?></small>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <div class="input-group mt-2">
                        <span class="input-group-text" style="background: linear-gradient(135deg, #fbbf24, #f59e0b); color: #000; font-weight: 700; border: 1px solid rgba(251, 191, 36, 0.3);"><strong><?= $NAMAN ?></strong></span>
                    <input type="text" id="estimation_2" class="form-control" placeholder="Your order will total" readonly>
                </div>
                </div>
                <div class="form-group">
                    <button type="submit" class="btn btn-primary" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none; padding: 12px 25px; border-radius: 12px; font-weight: 600; transition: all 0.3s ease; position: relative; overflow: hidden;">
...
<script>
    $(document).ready(function() {
        ...
        getPrice(price);
        $("#max_devices, #max_devices_2, #bulk, #duration, #duration_2, #game, #game_2").change(function() {
            getPrice(price);
        });
        function getPrice(price) {
            var device, durate, bulk, gprice, est278, result;
            if ($('#custom').val() == 'normal') {
                device = $("#max_devices").val();
                durate = $("#duration").val();
                bulk = $("#bulk").val();
                gprice = price[durate];
                if (gprice != NaN) {
                    est278 = (device * gprice);
                    result = (est278 * bulk);
                    $("#estimation").val(result);
                } else {
                    $("#estimation").val('Estimation error');
                }
              var device, durate, gprice, est278, result;
            } else if ($('#custom').val() == 'custom') {
                device = $("#max_devices_2").val();
                durate = $("#duration_2").val();
              //  bulk = 1; // assuming bulk is always 1 for normal keys
                gprice = price[durate];
                if (gprice != NaN) {
                    est278 = (device * gprice);
                    result = (est278);
                    $("#estimation_2").val(result);
                } else {
                    $("#estimation_2").val('Estimation error');
                }
            }
        }
    });
...
<script>
    $(document).ready(function() {
        $('#custom').val('normal').trigger('change');
