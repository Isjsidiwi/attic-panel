...
class UserModel extends Model
{
    ...
    protected $allowedFields = ['username', 'fullname', 'saldo', 'level', 'status', 'uplink', 'password', 'image', 'loginDevices', 'loginRsetTime'];
    ...
    public function getUserfile($select = "image")
    {
        $this->select($select);
        return $this->get()
            ->getResultObject();
    ...
    public function API_getUser()
    {
        ...
        $builder->select('CONCAT(users.id_users) as id, username, fullname, saldo, level, status, uplink, image');
